...
class VlMapLayer extends VlElement {
  ...
  async getFeatures() {
    return this.driver.executeScript(`return arguments[0].features.map((f) => new ol.format.GeoJSON().writeFeature(f));`, this);
  }

  async getFeature(id) {
    return this.driver.executeScript(`return new ol.format.GeoJSON().writeFeature(arguments[0].features.find((f) => f.getId() === arguments[1]));`, this, id);
  }
