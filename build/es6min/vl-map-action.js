import {vlElement} from '/node_modules/vl-ui-core/vl-core.js'; export class VlMapAction extends (vlElement(HTMLElement)) {
  connectedCallback() {
    ...
    this._layer=t, this._layerChangedCallback();
  } get action() {
    return this._action;
  } get _map() {
    if (this.parentNode) return this.parentNode.map;
  }activateAction() {
    this._action&&(this._map.activateAction(this._action), this.actionChanged());
  }actionChanged() {
    const t=new Event(VlMapAction.NEW_ACTION_EVENT_NAME); this.parentElement.dispatchEvent(t);
  }_layerChangedCallback() {
    this._computeAction(this._map, this.layer);
  }_createAction() {
    console.log('implementatie van de _createAction ontbreekt');
  }_computeAction(t, e) {
    let a; t&&e&&(a=this._createAction(e), this.parentElement.addAction(a), this.actionChanged()), this._action=a;
